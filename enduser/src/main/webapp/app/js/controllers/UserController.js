...
angular.module("self").controller("UserController", ['$scope', '$rootScope', '$location', "$state",
  ...
  function ($scope, $rootScope, $location, $state, UserSelfService, SchemaService, RealmService,
    ...
    $scope.initUser = function () {
      $scope.dynamicForm = {
        types: ['End user', 'Service provider'],
        plainSchemas: [],
        derSchemas: [],
        virSchemas: [],
        resources: [],
        ...
        selectedResources: [],
        selectedGroups: [],
        selectedChoreographies: [],
        selectedAuxClasses: [],
        groupSchemas: ['own'],
        selectedType: 'End user'
      };
      ...
      var initGroups = function () {
        ...
        GroupService.getGroups(realm).then(function (response) {
          ...
          for (var i in response.groups) {
            $scope.dynamicForm.groups.push({"rightKey": response.groups[i].key, "groupName": response.groups[i].name});
          }
          for (var i in response.choreographies) {
            $scope.dynamicForm.choreographies.push({"rightKey": response.choreographies[i].key, "groupName": response.choreographies[i].name});
          }
      ...
      var readUser = function () {
        UserSelfService.read().then(function (response) {
          ...
          // initialize type
          for (var index in $scope.user.roles) {
            var role = $scope.user.roles[index];
            if ("Service provider" === $scope.user.roles[index]) {
              $scope.dynamicForm.selectedType = role;
            }
          }
          ...
          $rootScope.viewGroups = "viewgroups";

          // initialize groups and choreographies
          var realm = $scope.user.realm || "/";
          GroupService.getGroups(realm).then(function (response) {
            for (var index in $scope.user.memberships) {
              if (indexOfResponse(response.groups, $scope.user.memberships[index]["groupName"])) {
                $scope.dynamicForm.selectedGroups.push(
                        {
                          "rightKey": $scope.user.memberships[index]["rightKey"].toString(),
                          "groupName": $scope.user.memberships[index]["groupName"]
                        });
              } else if (indexOfResponse(response.choreographies, $scope.user.memberships[index]["groupName"])) {
                $scope.dynamicForm.selectedChoreographies.push(
                        {
                          "rightKey": $scope.user.memberships[index]["rightKey"].toString(),
                          "groupName": $scope.user.memberships[index]["groupName"]
                        });
